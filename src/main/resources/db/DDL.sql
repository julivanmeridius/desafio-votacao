...
-- tabela de cadastramento de pauta
...
-- tabela para cadastramento de associados que irão votar
...
-- tabela para controle da sessao de votacao
...
-- Trigger para popular fecha_abertura = tempo_abertura + duracao_segundos
...
-- Índices (performance)
-- para buscar sessão por pauta/tempo (ex.: "sessão ativa" mais recente)
CREATE INDEX IF NOT EXISTS idx_sessao_votacao_pauta_tempo
    ON sessao_votacao(pauta_id, tempo_abertura);

-- tabela para cadastramento de votos dos associados
...
-- Índice para contagem rápida por sessão
CREATE INDEX IF NOT EXISTS idx_voto_sessao_votacao ON voto(sessao_votacao_id);

-- Trigger: valida voto (pauta_id consistente + sessão aberta e não encerrada por horário)
